...
// leaderboard_kof
let leaderboard_kof =
    `create table if not exists leaderboard_kof(
     id INT NOT NULL AUTO_INCREMENT,
     pos INT NOT NULL COMMENT '排名',
     name VARCHAR(100) NOT NULL COMMENT '用户名称',
     score VARCHAR(40) NOT NULL COMMENT '分数',
     latest_sub_date VARCHAR(40) NOT NULL COMMENT '最近提交时间',
     pos_change VARCHAR(40) NOT NULL COMMENT '排名改变',
     PRIMARY KEY(id)
    );`
...
// 获取 leader board
// pos, name, score, lates_sub_date, pos_change
exports.findLB_kof = () => {
  let _sql = `select * from leaderboard_kof order by pos desc `
  return query(_sql)
}
